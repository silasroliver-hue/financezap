...
async function dashboardRoute(req, res) {
  ...
  const { data: tx, error } = await sb
    .from("transactions")
    .select("id, kind, amount, category, occurred_on, description, source, created_at")
    .eq("user_id", userId)
  ...
  res.json({
    ...
    incomeCount,
    expenseCount,
    averageIncomeTicket: incomeCount > 0 ? income / incomeCount : 0,
    averageExpenseTicket: expenseCount > 0 ? expense / expenseCount : 0,
    expensePercentOfIncome: income > 0 ? (expense / income) * 100 : null,
